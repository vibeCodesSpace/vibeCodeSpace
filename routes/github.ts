  ...
  if (!appId || !privateKey || !webhookSecret) {
    ...
    app.get('/github/status', (req, res) => {
      res.json({
        ...
        instructions: 'Set GITHUB_APP_ID, GITHUB_APP_PRIVATE_KEY, and GITHUB_WEBHOOK_SECRET environment variables'
      });
    });
  ...
  try {
    ...
    app.post('/github/test/auto-fix', async (req, res) => {
      try {
        ...
        res.json({ 
          message: 'Auto-fix test would create a PR with formatting and linting fixes',
