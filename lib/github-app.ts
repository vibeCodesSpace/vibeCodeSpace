...
export class VibeCodeGitHubApp {
  ...
  private setupWebhooks() {
    ...
    // Build failures
    this.app.webhooks.on('check_run.completed', async ({ payload, octokit }) => {
      await this.handleCheckRunCompleted(payload, octokit);
    });

    // Workflow failures
    this.app.webhooks.on('workflow_run.completed', async ({ payload, octokit }) => {
      await this.handleWorkflowCompleted(payload, octokit);
    });

    // Error handling
    this.app.webhooks.onError((error) => {
      logger.error('GitHub webhook error', { error: error.message, stack: error.stack });
    });
  }
    ...
    try {
      ...
      await this.sendSlackNotification({
        ...
        attachments: [{
          color: 'danger',
          fields: [
            { title: 'Repository', value: `${repository.owner.login}/${repository.name}`, short: true },
            { title: 'Environment', value: deployment_status.environment, short: true },
            { title: 'Error Type', value: analysis.errorType, short: true },
            { title: 'Severity', value: analysis.severity, short: true },
            { title: 'Issue', value: `<${issue.data.html_url}|#${issue.data.number}>`, short: true },
            { title: 'Logs', value: `<${deployment_status.target_url}|View Logs>`, short: true }
          ]
        }]
      });
      ...
      await this.sendSlackNotification({
        ...
        attachments: [{
          ...
          fields: [
            { title: 'Repository', value: `${repository.owner.login}/${repository.name}`, short: true },
            { title: 'Environment', value: deployment_status.environment, short: true },
            { title: 'URL', value: `<https://www.vibecode.space|Visit App>`, short: true }
          ]
        }]
      });
  ...
  private async handlePush(payload: any, octokit: any) {
    ...
    if (pusher.name?.includes('bot') || commits.some((c: any) => c.message.includes('[skip ci]'))) {
      return;
    }
  ...
  private async handleIssueOpened(payload: any, octokit: any) {
    ...
    try {
      ...
      const category = await this.categorizeIssue(issue.title, issue.body || '');
      ...
      await this.sendSlackNotification({
        ...
        attachments: [{
          ...
          fields: [
            ...
            { title: 'View Logs', value: `<${workflow_run.html_url}|Open Workflow>`, short: true }
          ]
        }]
      });
    }
  ...
  private async analyzeDeploymentFailure(deploymentStatus: any) {
    ...
    if (description.includes('build failed') || description.includes('compilation')) {
      errorType = 'build-failure';
      severity = 'high';
      autoFixable = true;
    } else if (description.includes('timeout') || description.includes('failed to start')) {
      errorType = 'startup-timeout';
      severity = 'high';
    } else if (description.includes('health check')) {
      errorType = 'health-check-failure';
      severity = 'medium';
    } else if (description.includes('database') || description.includes('connection')) {
  ...
  private generateDeploymentFailureReport(analysis: any, deploymentStatus: any): string {
...
*This issue was automatically created by the VibeCode GitHub App*`;
    ...
    try {
      ...
      await this.sendSlackNotification({
        ...
        attachments: [{
          ...
          fields: [
            { title: 'Repository', value: repository.full_name, short: true },
            { title: 'Fix Type', value: analysis.fixType || analysis.analysisType, short: true },
            { title: 'PR', value: `<${pr.data.html_url}|#${pr.data.number}>`, short: true }
          ]
        }]
      });
  ...
  private generatePRTitle(analysis: any): string {
    switch (analysis.fixType || analysis.analysisType) {
      case 'linting': return 'Fix ESLint issues';
      case 'formatting': return 'Fix code formatting';
      case 'typescript': return 'Fix TypeScript compilation errors';
      case 'push-analysis': return 'Address potential issues from recent commits';
      default: return 'Automated fixes';
    }
  }
